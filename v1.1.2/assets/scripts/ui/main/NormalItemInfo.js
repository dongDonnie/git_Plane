...
cc.Class({
    ...
    refreshBtn: function () {
        ...
        if (itemData.byType == GameServerProto.PT_ITEMTYPE_CHEST || itemData.byType == GameServerProto.PT_ITEMTYPE_DROPPACKAGE) {
            ...
            this.node.getChildByName("btnVideo").active = GlobalVar.canShowVideo();
            this.node.getChildByName("btnShare").active = !GlobalVar.canShowVideo();
            this.node.getChildByName("btnUseMore").active = false;
            this.node.getChildByName("btnUseOne").active = false;
            this.btnConfirm.node.active = false;
        } else {
    ...
    onBtnShare: function () {
        if (this.itemID) {
            ...
            if (itemData != null) {
                let platfromApi = GlobalVar.getPlatformApi();
                if (platfromApi) {
                    platfromApi.shareNormal(138, function () {
                        GlobalVar.handlerManager().bagHandler.sendItemUseReq(itemData.Slot, 1, itemData.Type);
                    })
                } else if (GlobalVar.configGMSwitch()) {
                    GlobalVar.handlerManager().bagHandler.sendItemUseReq(itemData.Slot, 1, itemData.Type);
