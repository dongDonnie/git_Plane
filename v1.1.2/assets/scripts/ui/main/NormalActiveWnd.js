...
cc.Class({
        ...
        let updateModel = function (model, index) {
            ...
            if (rule) {
                if (rule.RuleID == GameServerProto.PT_AMS_RULEID_INVITE) {
                    ...
                    isShare = rule.RuleID == GameServerProto.PT_AMS_RULEID_SHARE;
                    isAD = rule.RuleID == GameServerProto.PT_AMS_RULEID_AD;
                    if (isShare || isAD){
                        ...
                        if (isShare && !GlobalVar.canShowShare()){
                            isShare = false;
                            isAD = true;
                        }else if (isAD && !GlobalVar.canShowVideo()){
                            isAD = false;
                            isShare = true;
                        }
                    }
    ...
    refreshContentType3: function (data) {
        ...
        nodeBtn.getChildByName("btnPurchase").getChildByName("spriteShare").active = false;
    ...
    refreshContentType4: function (data) {
        ...
        // 抽奖提示
        nodeBuy.getChildByName("btnPurchase").getChildByName("spriteVideo").active = false;
        nodeBuy.getChildByName("btnPurchase").getChildByName("spriteShare").active = false;
        if (data.Join[0] && data.Join[0].Join >= data.Act.LimitNum) {
