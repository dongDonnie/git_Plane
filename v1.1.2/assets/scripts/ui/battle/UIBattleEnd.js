...
cc.Class({
            ...
            if (platformApi) {
                ...
                if (endlessScore >= historyMaxScore) {
                    if (GlobalVar.isWechatPlat){
                        ...
                        // platformApi.submitUserData("key1", endlessScore);
                    }
                }
    ...
    initEndlessEndUINew: function () {
        ...
        if (nodeRecvGold.getChildByName("nodeShare").active){
            ...
            if (!GlobalVar.canShowVideo(true)){
    ...
    showSuperInduce: function () {
        ...
        console.log("superBannerProb" + platformApi.shareSetting.superBannerProb);
        console.log("SuperInduceCount=" + superInduceCount);

        if (!config.NEED_GUIDE &&
            GlobalVar.getShareSwitch() &&
            (cityFlagSwitch == 0 || cityFlag == 0) &&
            random < platformApi.shareSetting.superBannerProb &&
            superInduceCount < platformApi.shareSetting.superBannerMax &&
            (GlobalVar.isWechatPlat || GlobalVar.isQQPlayPlat)) {
