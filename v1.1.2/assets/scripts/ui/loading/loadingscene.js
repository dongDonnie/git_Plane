    ...
    onLoad: function () {
        ...
        if (this.firstLoading) {
            ...
            if (GlobalVar.isWanbaPlat){
                this.loadingBar.node.parent.getChildByName("labelGMQQ").active = true;
            }
                    ...
                    if (cc.sys.platform == cc.sys.WECHAT_GAME) {
                        ...
                        if (!GlobalVar.networkManager().connected) {
                            ...
                            platformApi.getNetWorkStatus(function (type) {
                                if (type === '-1') {
                                    platformApi.showToast("无网络链接", true, false, "确认", "取消", function () {
                                        GlobalVar.sceneManager().reStart();
                                    });
                                } else {
                                    platformApi.showToast("读取资源失败, 点击确认重试", true, false, "确认", "取消", function () {
                                        cc.director.loadScene('LoadingScene');
                                    });
                                }
                            })
